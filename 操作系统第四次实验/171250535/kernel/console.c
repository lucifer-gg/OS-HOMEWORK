	...
	default:
		if (p_con->cursor <
			...
			*p_vmem++ = ch;
			if (patternState == 0) {
				if(color==1){
					*p_vmem++ =SPECIAL_CHAR_COLOR_A;
				}else if(color==2){
						*p_vmem++ =SPECIAL_CHAR_COLOR_B;
				}else if(color==3){
						*p_vmem++ =SPECIAL_CHAR_COLOR_C;
					
				}else if(color==4){
						*p_vmem++ =SPECIAL_CHAR_COLOR_D;
					
				}else if(color==5){
						*p_vmem++ =SPECIAL_CHAR_COLOR_E;
					
				}else if(color==6){
						*p_vmem++ =SPECIAL_CHAR_COLOR_F;
				}else{
						*p_vmem++ = DEFAULT_CHAR_COLOR;
					}
			}
